...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBeetleAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBeetleAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBeetleAmount = strAnonymizeBeetleAmount.remove(strAnonymizeBeetleAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBeetleAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBeetleAmount*COIN) nMaxToAnonymize = nAnonymizeBeetleAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBeetleAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBeetleAmount));
        strAnonymizeBeetleAmount = strAnonymizeBeetleAmount.remove(strAnonymizeBeetleAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBeetleAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
