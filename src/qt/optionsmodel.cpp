...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBeetleAmount"))
        settings.setValue("nAnonymizeBeetleAmount", 1000);
    nAnonymizeBeetleAmount = settings.value("nAnonymizeBeetleAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBeetleAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBeetleAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBeetleAmount:
            return QVariant(nAnonymizeBeetleAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBeetleAmount:
            nAnonymizeBeetleAmount = value.toInt();
            settings.setValue("nAnonymizeBeetleAmount", nAnonymizeBeetleAmount);
            emit AnonymizeBeetleAmountChanged(nAnonymizeBeetleAmount);
            break;
        default:
            break;
        }
