...
namespace {
    class CBeetleAddressVisitor : public boost::static_visitor<bool> {
    private:
        CBeetleAddress *addr;
    public:
        CBeetleAddressVisitor(CBeetleAddress *addrIn) : addr(addrIn) { }
...
bool CBeetleAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CBeetleAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CBeetleAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CBeetleAddressVisitor(this), dest);
}

bool CBeetleAddress::IsValid() const {
...
CTxDestination CBeetleAddress::Get() const {
    if (!IsValid())
...
bool CBeetleAddress::GetKeyID(CKeyID &keyID) const {
...
bool CBeetleAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CBeetleSecret::SetKey(const CKey& vchSecret) {
...
CKey CBeetleSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CBeetleSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CBeetleSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CBeetleSecret::SetString(const std::string& strSecret) {
